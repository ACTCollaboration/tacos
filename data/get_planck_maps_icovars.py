...
# get some basics
rawpath = utils.data_dir_str('raw', 'planck')
mappath = utils.data_dir_str('map', 'planck')
covmatpath = utils.data_dir_str('covmat', 'planck')
...
for freq in freqs:
    for split in splits:
        ...
        omap_fn = utils.data_fn_str(type='map', instr='planck', band=freq, id='all', set=split)
        enmap.write_map(mappath + omap_fn, map_coadd.astype(args.odtype), extra=extra)

        ocoadd_fn = utils.data_fn_str(type='icovar', instr='planck', band=freq, id='all', set=split)
