...
class TestBandPass(unittest.TestCase):
    ...
    def test_integrate_signal(self):
    ...
    def test_integrate_signal_axis(self):
        ...
        out_3d = bp.integrate_signal(signal_3d, axis=-2)
        out_3d_exp = np.ones((3, 2)) * 5

        np.testing.assert_allclose(out_3d, out_3d_exp)
                         
    def test_integrate_signal_data(self):
        ...
        for instr in instr_band:
            for band in instr_band[instr]:
                band_int = data.Channel(instr=instr, band=band).bandpass.integrate_signal(np.ones(1))
