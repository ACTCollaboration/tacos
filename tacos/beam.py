...
class Beam():
    '''
    Beam instance contains normalized harmonic-space beam window functions.

    Parameters
    ----------
    beam : (nell) array
        Beam normalized such that peak value is 1. Assumes corresponds to
        ell = np.arange(beam.size + 1)
    '''

    def __init__(self, beam):
        
        self.beam = beam / np.max(beam)
